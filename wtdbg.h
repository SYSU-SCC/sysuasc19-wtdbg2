	...
	nhit = 0;
	{
		...
		while(rid<=qe + ncpu*nsize){
			// 唤起一批任务一,没用wait，主要是计算线程工作
			{
				...
				// 开始寻找
				got_size = ncpu;
				for(;got_size>0;got_size--){
					if(rid<qe){
						if(rank == 0 && !KBM_LOG && ((rid - qb) % 2000) == 0){ fprintf(KBM_LOGF, "\r%u|%llu", rid - qb, nhit); fflush(KBM_LOGF); }
					...
					if(mdbg->reg.closed == 0){
						cur_index++;
						mdbg->lt_buffer = LT_MPI_send_buffer + totalsize_send;
						totalsize_send+=encode_aux(mdbg->aux, mdbg->lt_buffer);;
						mdbg->reg.closed = 1;
					}
